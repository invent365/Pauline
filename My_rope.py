...
def yield_repr(seq_len, dim, dimensions):
...
def inject(seq_len, dim):
    sin_vector,cos_vector,dimensions = yield_repr(seq_len,dim)
    x = torch.zeros((seq_len, dim, dim)) #cache
...
def yield_template(input_:torch.Tensor, seq_len:int, dim:int): #input should be of size(seq_len,dim) at the very least.
    template = inject(seq_len,dim) #seq_len,dim,dim
    out = torch.einsum("ijj, ij->ij", template,input_) 
    return out
